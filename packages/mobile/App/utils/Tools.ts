...
import {DefaultCurrency, OrderStatus, PaymentTypes, THUMBNAIL_URL} from './constants';
...
export default class Tools {
    ...
    // format currency
    static getPrice = (price: number, weight = 1, qty = 1, extra = {}) => {
        let defaultCurrency = {
            isShippingPriceIncluded: 0,
            shippingPrice: 0,
        };

        if (extra?.isShippingPriceIncluded && extra?.shippingPrice) {
            defaultCurrency = {...defaultCurrency, ...extra};
        } else {
            let generalCurrency = store?.getState()?.general?.country;
            if (generalCurrency) {
                defaultCurrency = {
                    isShippingPriceIncluded: generalCurrency?.isShippingPriceIncluded,
                    shippingPrice: _has(generalCurrency, 'currencies[0].shippingPrice')
                        ? generalCurrency?.currencies[0]?.shippingPrice
                        : 0,
                };
            }
        }
        if (
            defaultCurrency?.isShippingPriceIncluded &&
            defaultCurrency?.shippingPrice &&
            defaultCurrency?.shippingPrice > 0
        ) {
            price = price * qty + Math.ceil(qty * weight) * defaultCurrency?.shippingPrice;
            return price;
        }
        return price * qty;
    };
    static getPriceWithCurrecyFormatted = (price: number, weight = 1, qty = 1, extra = {}) => {
        let defaultCurrency = {
            currencyCode: 'USD',
            isShippingPriceIncluded: 0,
            shippingPrice: 0,
        ...
        return Tools.getCurrecyFormatted(price, defaultCurrency);
    };
    static getCurrecyFormatted = (price: number, extra = {}) => {
        let defaultCurrency = {
            currencyCode: 'USD',
            exchangeRate: 1,
        };

        if (extra?.currencyCode && extra?.exchangeRate) {
            defaultCurrency = {...defaultCurrency, ...extra};
        } else {
            let generalCurrency = _has(store?.getState()?.general?.country, 'currencies[0].currency')
                ? store?.getState()?.general?.country?.currencies[0]?.currency
                : defaultCurrency;
        ...
        // let preferredCurrency = store.getState().auth.preferredCurrency;
    ...
    static getCartSubTotal = cart => {
        ...
        cart.cartItems.map(c => {
            if (c.status) {
                amount = amount + Tools.getPrice(c.product.regularPrice, c.product.weight, c.quantity);
            }
        });
        return amount;
    };
    ...
    static getCartProduct = (product, item, qty) => {
        // console.log(getProductImage(item.colour.files[0]?.fileName, 400));
        return {
            title: product?.title,
    ...
    static getPaymentTypes = (paymentTypeId: number): object => {
        return PaymentTypes[paymentTypeId];
    };
    static getOrderStatus = (orderStatus: string): string => {
